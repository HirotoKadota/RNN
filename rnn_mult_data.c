...
int per = (int) (TIME_LEN / ONE_PERIOD_LEN);
int getrand();
double rand_normal(double mu, double sigma);
double mean_squared_error(const double [][TIME_LEN]);
void draw_graph(FILE *fp, double y[][TIME_LEN], int num);
...
int main(){
	...
	for(i = 0; i < TIME_LEN; i++){
		x[0][i] = 0.8 * sin(2 * per * M_PI * i / TIME_LEN) + rand_normal(0.0, SD);
		x[1][i] = 0.8 * sin(2 * per * M_PI * 2 * i / TIME_LEN) + rand_normal(0.0, SD);
		y[0][i] = 0.8 * sin(2 * per * M_PI * (i + 1) / TIME_LEN);
		y[1][i] = 0.8 * sin(2 * per * M_PI * 2 * (i + 1) / TIME_LEN);

		x_[0][i] = 0.8 * sin(2 * per * M_PI * i / TIME_LEN) + rand_normal(0.0, SD);
		x_[1][i] = 0.8 * sin(2 * per * M_PI * 2 * i / TIME_LEN) + rand_normal(0.0, SD);
	}
...
void draw_graph(FILE *fp, double y[][TIME_LEN], int num){
    ...
    if(num == 0){
        if( y[m + 1][0] - 0.8 * sin(2 * 2 * M_PI / TIME_LEN) != 0.0 ){
